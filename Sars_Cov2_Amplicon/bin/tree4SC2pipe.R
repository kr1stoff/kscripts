...
# [20220113 update] single sample phylogenetic tree, mark
...
if (max_nchar_label < 50) {
  ggtree_xlim_right <- longest_node_level * 1
  p2_xlim_fold <- 1.4
  p14_width <- 9
  p23_width <- 9
  pm_width <- 15
} else if (max_nchar_label > 50 & max_nchar_label < 100) {
  ...
  ggtree_xlim_right <- longest_node_level * 1.8
  p2_xlim_fold <- 2.4
  p14_width <- 15
  p23_width <- 21
  pm_width <- 21
...
ggsave(plot = p1, filename = "rectangular.svg",
       width = p14_width, height = 5.7, dpi = 500, path = output_dir)
...
ggsave(plot = p2, filename = "rectangular_bl.svg",
       width = p23_width, height = 5.7, dpi = 500, path = output_dir) #width = 9
#~ 4 slanted
...
ggsave(plot = p4, filename = "slanted.svg",
       width = p14_width, height = 5.7, dpi = 500, path = output_dir) #width = 9
#~ 3 circular
p3 <- ggtree(tree_withGroup, layout="circular",
             branch.length = "none", root.position = 10,
             aes(color=group)) + #
  geom_tiplab(hjust=-0.05, angle=0) +
  geom_tippoint(color="black", alpha=1/4) +
  geom_nodepoint(color="black", alpha=1/4) +
  scale_color_manual(values = c("red", "black")) +
  theme_tree2() +
...
ggsave(plot = p3, filename = "circular.svg",
       width = p23_width, height = 5.7, dpi = 500, path = output_dir) #width = 9
#~ multi graphs
# pm <- plot_grid(p1, p2, p4, ncol = 1, nrow = 3)
# ggsave(plot = pm, filename = "jagsaw.svg",
#        width = pm_width, height = pm_width, dpi = 500, path = output_dir) #width = 15
