...
options(warn = -1)
my.prompt <- paste(paste0(rep("#", 50), collapse = ""), "PROMPT", 
...
spec <- matrix(
  c("input", "i", 1, "character", "Input file, like OTU/Species/Genus abundance table.",
    "group", "g", 1, "character", "Group information of Input samples.", 
    "outpath", "o", 2, "character", "[optional] Result files path. [default: ./16S_keyan_workpath/results]",
    "summary", "s", 0, "logical", "[optional] Summary each sample sequencing reads number, no analysis results",
    "min_reads_threshold", "m", 2, "integer", "[optional] Minimum sequencing reads number, discard Low-Abundance Sample(< threshold). [default: 1000]",
    "help", "h", 0, "logical", "Display help infomation."
  ),
  byrow = T, ncol = 5)

opt <- getopt(spec)
if (!is.null(opt$help) || is.null(opt$input) || is.null(opt$group)) {
  cat(getopt(spec, usage = T))
  q(status = 1)
}
#~ result path
if (is.null(opt$outpath) ) { opt$outpath <- "./16S_keyan_workpath/results" }
#~ minimum reads number threshold
if (is.null(opt$min_reads_threshold)) {
  threshold.reads_number <- 1000
} else {
  threshold.reads_number <- opt$min_reads_threshold
}
#~ print arguments
cat(my.prompt)
cat(paste("Input: ", opt$input, sep = '\t'), 
    paste("Group: ", opt$group, sep = '\t'), 
    paste("Outpath: ", opt$outpath, sep = '\t'),
    paste("Minimum Reads Number Threshold: ", threshold.reads_number, sep = '\t'),
    "Configuration is successful!", sep = '\n')
###############################################################################
cat(my.prompt)
print(paste("INFO", date(), "16S DIVERSITIES ANALYSIS", sep = ' - '))
print(paste("INFO", date(), "Library Packages", sep = ' - '))
...
###############################################################################
print(paste("INFO", date(), "Create Directory & Read Tables", sep = ' - '))
## otu table
raw.data <- read.table(opt$input, sep = '\t', header = T, stringsAsFactors = F,
                       check.names = F, row.names = 1) %>% t() %>% as.data.frame()
## [20210419] sample reads summary
if (!is.null(opt$summary)) {
  cat(my.prompt, "\n")
  print(rowSums(raw.data)[order(rowSums(raw.data))])
  cat("\n")
  print(summary(rowSums(raw.data)))
  cat(my.prompt, "\n")
  q(save = "no", status = 0)
}
## reads count threshold
#~ [20210419] reads number threshold
if (length(which(rowSums(raw.data) <= threshold.reads_number)) != 0) {
  otu.table <- raw.data[-c(which(rowSums(raw.data) <= threshold.reads_number)),]
  otu.table <- otu.table[,-c(which(colSums(otu.table) == 0))]
} else {
  otu.table <- raw.data
}

## group 
otu.group <- read.table(opt$group, sep = '\t', header = T, row.names = 1, 
                        check.names = F, stringsAsFactors = F)
group.unique <- unique(otu.group$group)
group.count <- length(group.unique)
## otu 
otu.table$group <- otu.group[rownames(otu.table),]
otu.table <- filter(otu.table, !is.na(group))
otu <- otu.table[,-ncol(otu.table)]
sample.names <- rownames(otu)
sample.count <- length(sample.names)

## result path
result.path <- opt$outpath
...
meta.group <- data.frame(names=rownames(otu.table), group=otu.table$group)
...
## real step
min.depth <- min(rowSums(otu))
if (min.depth < 1000) {real_step <- 100 } else {real_step <- signif(min.depth/6, digits = 2)}

## rarefy first  
print(paste("INFO", date(), "Rarefy Taxa Table & Prepare metagenomeSeq Input File", sep = ' - '))
...
print(paste("INFO", date(), "Alpha Diversities Indices", sep = ' - '))
...
print(paste("INFO", date(), "Alpha Diversities Indices Boxplot", sep = ' - '))
...
print(paste("INFO", date(), "Alpha Diversities Indices Difference", sep = ' - '))
...
print(paste("INFO", date(), "Alpha Diversities Dilution Curve", sep = ' - '))
...
print(paste("INFO", date(), "Top 20 Species Abundance", sep = ' - '))
otu.use = t(otu.rare)
if(dim(otu.use)[1] > 20){
...
print(paste("INFO", date(), "Multi-sample Rank-abundance", sep = ' - '))
...
p <- ggplot(data = abundance.m, aes(x=rank, y=value, group=sample, color=sample)) + geom_line() + 
  ...
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5)) + 
  scale_y_continuous(labels = scientific, limits = c(1e-5, 1e-1), trans = "log10")
ggsave("Rank_abundance.png", path = Alpha_diversity.path)

## Species Accumulation Curves
print(paste("INFO", date(), "Species Accumulation Curves", sep = ' - '))
...
print(paste("INFO", date(), "Taxa Distribution Venn", sep = ' - '))
...
print(paste("INFO", date(), "Bray-Curtis Distance & Distance Matrix Heatmap", sep = ' - '))
...
print(paste("INFO", date(), "UPGMA", sep = ' - '))
...
print(paste("INFO", date(), "PCA & adonis(PERMANOVA)", sep = ' - '))
...
## PCoA
print(paste("INFO", date(), "PCoA", sep = ' - '))
...
## NMDS
print(paste("INFO", date(), "NMDS", sep = ' - '))
...
print(paste("INFO", date(), "Anosim, MRPP, Adonis", sep = ' - '))
...
## ranksum & T-test
print(paste("INFO", date(), "Different Species Ranksum & T-tests", sep = ' - '))
...
print(paste("INFO", date(), "metagenomeSeq", sep = ' - '))
