...
# @Author           : mengxf
# @version          : v1.2
# @LastModified     : 2023/4/21
# @description      : FASTQ去引物软件
...
class FQTrimPrimer():
    ...
    def my_config(self):
        ...
        self.dir_conf = Path(__file__).resolve().parent.joinpath("conf")
    ...
    def alignment(self):
        logging.info("比对")
        thrd = self.parallel if self.parallel != 1 else 8
        cml = f"""
{self.bwa} mem -t {thrd} -M -Y -R '@RG\\tID:trim\\tSM:trim' {self.ref} {self.dir_tmp}/prepared.fq > {self.dir_tmp}/raw.sam
        """
        logging.debug(cml)
        os.system(cml)
